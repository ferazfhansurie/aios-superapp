...
export function CfoFinanceCard() {
  ...
  return <section className="aios-fade-in w-full rounded-2xl border border-[var(--color-border)] bg-[var(--color-panel)]/55 p-4">
    <FinanceGlance />
    {!f.monthMismatch && <div className="mt-4 grid grid-cols-2 gap-3 text-[11px] sm:grid-cols-5">
      <Metric label="gross cash" value={formatMyr(f.cash)} />
      <Metric label="card debt" value={formatMyr(f.card_debt)} />
      <Metric label="net cash" value={formatMyr(f.netCash)} strong />
